...
export default function Sidebar() {
  ...
  const SidebarContent = () => (
    <div className="flex flex-col h-full">
      {/* Logo */}
      <div className={`flex items-center gap-2.5 px-4 py-5 border-b border-[#C0D2DE]/40 ${collapsed ? "justify-center px-2" : ""}`}>
        <div className="w-8 h-8 bg-[#036D7D] rounded-lg flex items-center justify-center flex-shrink-0">
          <Droplets size={16} className="text-white" />
        </div>
        {!collapsed && (
          <div>
            <p className="font-display text-[#021734] text-base font-bold leading-tight">RakSetu</p>
            <p className="text-[10px] text-[#036D7D] font-medium">{portalLabel}</p>
          </div>
        )}
      </div>
      ...
      {/* User + Logout */}
      <div className={`border-t border-[#C0D2DE]/40 p-3 ${collapsed ? "flex flex-col items-center gap-2" : ""}`}>
        {!collapsed && (
          <div className="flex items-center gap-2.5 px-2 py-2 mb-1">
            <div className="w-7 h-7 rounded-full bg-[#036D7D] text-white text-xs flex items-center justify-center font-bold flex-shrink-0">
              {currentUser?.name?.[0] || "U"}
            </div>
            <div className="min-w-0">
              <p className="text-xs font-semibold text-[#021734] truncate">{currentUser?.name}</p>
              <p className="text-[10px] text-[#021734]/40 capitalize">{role}</p>
            </div>
          </div>
        )}
        <button
          onClick={handleLogout}
          className={`flex items-center gap-2 w-full px-3 py-2 text-xs text-[#021734]/50 hover:text-red-600 hover:bg-red-50 rounded-lg transition-all ${collapsed ? "justify-center" : ""}`}
        >
          <LogOut size={14} />
          {!collapsed && "Sign Out"}
        </button>
  ...
  return (
    <>
      ...
      <AnimatePresence>
        {mobileOpen && (
          <motion.aside
            ...
            className="md:hidden fixed left-0 top-0 h-full w-64 bg-[#FBFBFD] border-r border-[#C0D2DE]/60 z-50"
          >
            <SidebarContent />
          </motion.aside>
        )}
      ...
      <motion.aside
        ...
        transition={{ type: "spring", stiffness: 300, damping: 30 }}
        className="hidden md:flex flex-col h-full bg-[#FBFBFD] border-r border-[#C0D2DE]/60 relative flex-shrink-0"
      >
        <SidebarContent />
      </motion.aside>
    </>
