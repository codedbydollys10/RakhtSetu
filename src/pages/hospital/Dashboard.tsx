  ...
  return (
    <DashboardLayout>
      <PageHeader
        ...
        actions={
          <Link to="/hospital/requests/create" className="flex items-center gap-2 px-4 py-2 bg-[#036D7D] text-white rounded-lg text-sm font-medium hover:bg-[#057080] transition-colors">
              ...
              active.map((req, i) => (
                ...
                >
                  <Link to={`/hospital/requests`}>
                    <div className={`bg-white rounded-xl border p-4 hover:shadow-sm transition-all ${req.urgency === "CRITICAL" ? "border-red-200 bg-red-50/20" : "border-[#C0D2DE]/60"}`}>
                      <div className="flex items-start justify-between gap-3">
                        ...
                        <div className="text-right flex-shrink-0">
                          <span className="text-xs px-2 py-0.5 bg-[#C0D2DE]/30 text-[#021734]/60 rounded-full">{req.status}</span>
            ...
            <div className="space-y-3">
              {notifications.slice(0, 3).map((n) => (
                <div key={n.id} className={`flex gap-3 text-xs ${!n.read ? "opacity-100" : "opacity-60"}`}>
                  <div className="w-1.5 h-1.5 rounded-full bg-[#036D7D] mt-1.5 flex-shrink-0" />
                  <div>
